...
EulerAngles BasicAiming::update(ArmorFrameInput detection, cv::Mat *debug_img, std::string *debug_msg) {
    return impl->update(std::move(detection), debug_img, debug_msg);
}
