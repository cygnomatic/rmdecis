...
//获取二值化图像
inline Mat GrayFilter(Mat image, int mode, int grayThres)
{ //grayThres可视为kThreashold，识别red是1，否则为0
...
//struct fpoints
inline Mat enhanceContour(Mat image)
{ //腐蚀膨胀
    int niters = 2;
...
vector<ArmorPredResult> detect(Mat img, CLOR clour)
{
    ...
    for (int i = 0; i < index - 1; i++)
    {
        for (int j = i + 1; j < index; j++)
        {
            ...
            //// cout<<endl;
            if (arcle(point_array[i], point_array[j]) <= 0.5 && shaperate(point_array[i], point_array[j]) <= 1.64 &&
                shaperate(point_array[i], point_array[j]) >= 0.7 && arearate(point_array[i], point_array[j]) <= 2.54 &&
                arearate(point_array[i], point_array[j]) >= 1)
            {
                // cout << "we get it" << endl << endl;
                my.armor_type = ArmorID::STANDARD_3;
                ppt = true;
                minm = value;
                point_near[0] = i;
                point_near[1] = j;
